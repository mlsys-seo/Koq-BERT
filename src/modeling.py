...
build_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../build_lib/build/lib')
  ...
  for layer_idx in range(num_hidden_layers):
    with tf.variable_scope("layer_%d" % layer_idx):
      if tf.app.flags.FLAGS.mix:
        if layer_idx == 8:
          tf.flags.FLAGS.floatx = 'float16'
          layer_input = tf.cast(prev_output, tf.flags.FLAGS.floatx)
      ...
      layer_output = transformer_op_module.bert_transformer(
                ...
                batch_size=tf.app.flags.FLAGS.batch_size, from_seq_len=512, to_seq_len=512, head_num=16, size_per_head=64)  
